...
class larsWFanalogSFCView extends Ui.WatchFace {

    static const HOUR_TICK_LENGTH = 10;
    static const MINUTE_TICK_LENGTH = 6;	
	static const MINUTE_MARK_COLOR = Gfx.COLOR_DK_GREEN;
	//static const MINUTE_MARK_COLOR = Gfx.COLOR_LT_GRAY;
	static const SECOND_HAND_COLOR = Gfx.COLOR_DK_BLUE;
	...
	function onUpdate(dc) {
		...
		var dateStr = clockTime.month + " " + clockTime.day.format("%02d");
		...
		// draw the background
		dc.drawBitmap(40,0,resBackground);
		...
		dc.setColor(Gfx.COLOR_LT_GRAY, Gfx.COLOR_TRANSPARENT);
        dc.drawText(70, 170, Gfx.FONT_SMALL, getMinHR(), Gfx.TEXT_JUSTIFY_RIGHT);
	...
	function drawHands(dc, clockTime, drawSecondHand) {
        ...
        //drawHand(dc, hourHand, DeviceOverride.HOUR_HAND_LENGTH, 6, Gfx.COLOR_DK_GRAY);
        drawHand(dc, hourHand, DeviceOverride.HOUR_HAND_LENGTH, 8, Gfx.COLOR_WHITE);

        // draw the minute
        minuteHand = (clockTime.min / 60.0) * Math.PI * 2 - degreesToRadians(90);
        drawHand(dc, minuteHand, DeviceOverride.MINUTE_HAND_LENGTH, 8, Gfx.COLOR_WHITE);
